...
def normalize_img(
    ...
    normalized_img = np.empty_like(img_array, dtype=np.float32)

    # Preserve ImageNet normalization for the first 3 channels.
    ...
    # Additional channels.
    # Your 4th channel is uint8, so scale it approximately to [-1, 1].
    for i in range(3, num_channels):
        normalized_img[..., i] = (
            img_array[..., i].astype(np.float32) - 127.5
        ) / 127.5
